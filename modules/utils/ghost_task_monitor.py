...
from core.event_bus import ShadowFoxEventBus, ShadowFoxEvent, EventType

# Import the original GhostThreadsEngine
from shadowfox.agents.ghost_threads import GhostThreadsEngine, ShadowPersona

class GhostThreadMonitor:
    def __init__(self, max_idle_time=300):
        self.logger = logging.getLogger("GhostThreadMonitor")
        ...
        while True:
            ...
            for task in asyncio.all_tasks():
                ...
                if not task.done():
                    ...
                    if frame:
                        code_info = frame[0].f_code.co_name
                        self.logger.warning(f"🧟 Ghost? Task '{code_info}' still running...")
                    else:
                        self.logger.info(f"Task {task.get_name()} ok")
